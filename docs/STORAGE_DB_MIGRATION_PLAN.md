...
> Created: 2026-09-08 · Updated: 2026-09-09 · Status: **P0–P6 เสร็จครบทุกข้อ ทดสอบกับโปรเจกต์จริงแล้วทั้งหมด** — P6-6 (tank gallery + Google login) เป็นข้อสุดท้าย · ข้อตัดสินใจหลักล็อกครบแล้ว (§0)
...
| `rls` / `share` / `sync` / `share-ui` | 18/18 · 30/30 · **26/26** · 14/14 | ของเดิมไม่พังจากการเปลี่ยน (sync เพิ่ม 6 ข้อคุมหลายตู้/การลบตู้) |
...
| `public` | **เปิดใน UI แล้วตอน P6-6** — ตอน P6-3 ยังไม่เปิด เพราะยังไม่มีอะไรมาแสดงรายการตู้ | ตามที่ §6.4 เขียนไว้เอง: ห้ามเปิดสาธารณะก่อนมีปุ่มรายงาน + `hidden_by_admin` (มาแล้วที่ P6-5) **และก่อนมีที่ให้ตู้ public ไปโผล่** — ไม่งั้นมันคือตู้ unlisted ที่เดา address ได้ ซึ่งแย่กว่าตัวเลือกข้าง ๆ มัน (รายการมาแล้วที่ P6-6) |
...
4. **`save()` คืนค่าเสร็จก่อนที่การเปลี่ยนแปลงจะเข้าคิว** (เจอตอน P6-6) — `repos.tank.save()` / `repos.sprites.put()` เรียก `onWrite` แบบ fire-and-forget แต่ `onWrite` คือการเขียนแถวลง outbox ซึ่งเป็น IndexedDB transaction อีกตัวหนึ่ง · แปลว่ามีช่วงสั้น ๆ ที่งานถูกบันทึกลงเครื่องแล้วแต่**ยังไม่มีใครรู้ว่าติดค้างต้องส่งขึ้น server** — ปิดแท็บในจังหวะนั้น งานยังอยู่ครบแต่จะเงียบไปจนกว่าจะเซฟ record เดิมอีกครั้ง · แก้ด้วยการ `await` callback ทั้งสามตัว (`onWrite` ของ sprite/tank และ `onDelete`) แล้วให้ `attachSync` คืน promise แทนที่จะ `void` ทิ้ง · มีเทสต์คุม 3 ข้อที่ตรวจ**ลำดับ** ไม่ใช่จังหวะเวลา — เวอร์ชันแรกของเทสต์เช็คว่า "หลัง N microtask แล้ว resolve หรือยัง" ซึ่งผ่านทั้งที่แก้และไม่แก้ จับอะไรไม่ได้เลย จนต้องเขียนใหม่

**บทเรียนเรื่องสคริปต์เทสต์ (จาก P6-6 เหมือนกัน):** สคริปต์ smoke ห้ามใช้ id คงที่กับ record ที่ client เป็นคนตั้ง id เอง · `inst_gallery_probe` ที่รันค้างไว้จากรอบที่ crash ทำให้รอบต่อ ๆ มาชนกับแถวของ**บัญชีอื่น** → RLS ปฏิเสธ upsert (`42501`) แทนที่จะเขียนทับ → outbox retry ไม่รู้จบ → ตู้ขึ้นว่างเปล่าโดยไม่มีอะไรบนจอบอกสาเหตุ · ตอนนี้ทุกสคริปต์ mint id ต่อรอบแล้ว · และอาการนี้โผล่มาเป็น React key warning (`id: undefined`) กับ `0 fish` บนการ์ด ก่อนจะเจอ error จริงที่ซ่อนอยู่ใน `console.warn` ซึ่งสคริปต์ไม่ได้นับเป็น error
...
- [x] **P6-6** tank gallery + Google login — `gallery_tanks()` เป็น SECURITY DEFINER ที่**ไม่คืน `user_id` และไม่คืน `share_slug`** (สองอย่างนี้อยู่ในตาราง `tanks` · slug หลุดทีเดียวคือตู้ unlisted ทั้งโปรเจกต์เปิดหมด) · [`TankGalleryDialog.tsx`](../src/components/tank/TankGalleryDialog.tsx) เปิดจากแท็บ Share · เปิดตู้แล้วไปโผล่ที่ [`SharedTankView`](../src/components/tank/SharedTankView.tsx) ตัวเดิม (ตู้ public กับตู้ที่แชร์ตรงลงจอเดียวกัน) · ปุ่มรายงานอยู่**ทั้งบนการ์ดและในหน้าดูตู้** — คนที่เปิดดูแล้วเท่านั้นที่รู้ว่าตู้นั้นควรถูกรายงานไหม · Google login เป็น `signInWithOAuth` ที่กลับเข้า `onAuthStateChange` เดิม ไม่มี path ที่สองให้ดูแล · เปิดปุ่มด้วย `VITE_GOOGLE_SIGN_IN=true` หลังตั้ง provider ใน dashboard แล้ว

**P6-6 — สิ่งที่ตัดสินใจไว้ (อ่านก่อนแก้)**

| หัวข้อ | ที่ทำ | ทำไม |
| --- | --- | --- |
| การ์ดตู้ไม่มีรูป | มีแค่ชื่อ + จำนวนปลา + ลำดับเวลา | จะวาดรูปตัวอย่างต้องดึงตู้ทั้งใบ (ทุก instance + ทุก sprite ที่ใช้) ต่อการ์ดหนึ่งใบ = เกือบเท่าโหลดหน้าเว็บหนึ่งครั้งต่อการ์ด เพื่อให้ได้สิ่งที่คลิกเดียวก็เห็น · sprite gallery มีรูปได้เพราะ thumbnail คือ 16×16 px ที่มากับ row อยู่แล้ว |
| ไม่มีปุ่ม copy ตู้ | ตู้ copy ไม่ได้ ต่างจาก sprite | sprite คือรูปวาด การก๊อปตรงไปตรงมา · ตู้คือ*ที่*ที่คนเลี้ยงปลาของเขา — ปลาในนั้นไม่ใช่ของเรา และ sprite ที่ปลาใช้อยู่ในคลังของเจ้าของ ไม่ได้อยู่ในตู้ · ใครอยากได้ปลาที่เห็นให้ไปก๊อปที่ sprite gallery ซึ่งทั้งหน้าทำมาเพื่อการก๊อปอยู่แล้ว |
| ตู้ public ยังมีลิงก์ใช้ได้ | เปลี่ยนเป็น public แล้ว slug เดิมยังใช้ได้ | public เปิดกว้างกว่า unlisted อยู่แล้ว · การดึงลิงก์คืนจากคนที่ถืออยู่แล้วคือความประหลาดใจที่ไม่มีใครขอ |
| guest เปิด gallery ได้ | ไม่ล็อกอินก็กดดูรายการและเปิดตู้ได้ · รายงานต้องล็อกอิน | `gallery_tanks` ให้สิทธิ์ `anon` ตั้งใจ · แต่รายงานต้องมีเจ้าของ (RLS ของ `content_reports` เป็นคนบังคับ ไม่ใช่ UI) |
| Google login เป็น flag | `VITE_GOOGLE_SIGN_IN=true` ถึงจะโชว์ปุ่ม | Supabase ไม่มี endpoint สาธารณะให้ถามว่าโปรเจกต์เปิด provider ไหนไว้ · ทางเลือกอีกทางคือโชว์ปุ่มแล้วให้มันพัง ซึ่งเอา error ไปวางหน้าผู้ใช้สำหรับ setting ที่มีแต่เจ้าของโปรเจกต์แก้ได้ · magic link ไม่ต้องตั้งอะไร เลยเปิดตลอด |
